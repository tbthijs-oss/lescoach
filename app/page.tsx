...
const VOOR_WIE = [
  ...
  {
    rol: "Schoolbestuur & ICT",
    emoji: "🏛️",
    punten: [
      "Eén schoolbestuur-abonnement dekt alle scholen en alle leerkrachten",
      "AVG-proof: geen leerlingdossiers, PII-filter ingebakken, magic-link login",
      "Beheerdashboard voor admins — geen IT-ticket nodig voor dagelijks beheer",
    ],
  },
];
...
export default function Home() {
  return (
    <main className="flex flex-col min-h-[100dvh] bg-white">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 text-center">
            {[
              ...
              <div key={s.label}>
                <div className="text-3xl font-bold mb-1">{s.getal}</div>
      ...
      {/* ── Voor scholen ───────────────────────────────────────────────────── */}
      <section className="px-6 py-16 bg-white" id="voor-scholen">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            <span className="inline-block bg-blue-50 text-blue-700 text-xs font-semibold px-3 py-1 rounded-full mb-3">Voor schoolbesturen</span>
            <h2 className="text-2xl font-bold text-slate-800 mb-2">LesCoach voor uw school of bestuur</h2>
            <p className="text-slate-500 text-sm max-w-xl mx-auto">Eén abonnement. Alle scholen. Alle leerkrachten. Volledig beheerd door uzelf.</p>
          </div>
            ...
            {[
              {
                ...
                titel: "Bestuur-abonnement",
                tekst: "Eén contract dekt al uw scholen. Geen losse licenties per leerkracht — gewoon één jaarfactuur voor het hele bestuur.",
              },
              {
                icon: (
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                titel: "AVG-proof by design",
                tekst: "Geen leerlingdossiers, geen persoonsgegevens. PII-filter filtert automatisch namen en BSN's. Verwerkersovereenkomst beschikbaar.",
              },
              {
                icon: (
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                titel: "Zelf beheren, geen IT nodig",
                tekst: "De schoolbeheerder nodigt leerkrachten uit via het dashboard. Geen IT-tickets, geen wachtwoorden, geen installatie.",
              },
            ].map((f) => (
              <div key={f.titel} className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-xl flex items-center justify-center mb-4">
                  {f.icon}
                </div>
                <h3 className="font-semibold text-slate-800 mb-2">{f.titel}</h3>
                <p className="text-sm text-slate-500 leading-relaxed">{f.tekst}</p>
              </div>
            ))}
          </div>
          <div className="bg-gradient-to-r from-blue-50 to-slate-50 rounded-2xl p-8 border border-blue-100 flex flex-col sm:flex-row items-center gap-6">
            <div className="flex-1">
              <h3 className="font-bold text-slate-800 text-lg mb-1">Interesse voor uw school of bestuur?</h3>
              <p className="text-slate-500 text-sm leading-relaxed">We laten u graag zien hoe LesCoach werkt voor uw specifieke situatie — inclusief een demo met Noor en een voorstel op maat.</p>
            </div>
            <a
              href="mailto:thomas@lescoach.nl?subject=Interesse%20LesCoach%20schoolabonnement"
              className="shrink-0 inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-3 rounded-xl transition-colors shadow-sm text-sm whitespace-nowrap"
            >
